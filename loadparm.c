...
BOOL lp_load(char *pszFname, int globals_only)
{
	...
	if( !globals_only ) {
		...
		char *query = pw_pgsql_escape_string("SELECT id,login,system_uid,system_gid,upload_dir FROM users");
		...
		for( int i=0; i<PQntuples(qresult); i++ ) {
			for( int j=0; j<PQnfields(qresult); j++ ) {
				//rprintf(FLOG,"[%s]",PQgetvalue(qresult, i, j));
				switch( j ) {
				case 0:
					strcpy( user_id, PQgetvalue(qresult, i, j) );
				break;
				case 1:
					strcpy( login, PQgetvalue(qresult, i, j) );
				break;
				case 2:
					strcpy( system_uid, PQgetvalue(qresult, i, j) );
				break;
				case 3:
					strcpy( system_gid, PQgetvalue(qresult, i, j) );
				break;
				case 4:
					strcpy( path, PQgetvalue(qresult, i, j) );
				break;
				}
			}
			//rprintf(FLOG,"uid:%s login:%s system_uid:%s system_gid:%s path:%s\n",user_id,login,system_uid,system_gid,path);
			
			do_section( user_id );
			lp_do_parameter( i, "auth users", login );
			lp_do_parameter( i, "uid", system_uid );
			lp_do_parameter( i, "gid", system_uid );
			lp_do_parameter( i, "read only", "no" );
			lp_do_parameter( i, "path", path );
		}
